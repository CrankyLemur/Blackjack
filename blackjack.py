...
class Blackjack:
    ...
    def __init__(self):
        ...
        self.split_cards = []
        self.upside_down_card = Card(0, "?")
        self.face_cards = ["J", "Q", "K", "A"]

        # Get the absolute path for the leaderboard.txt file.
        self.file_path = os.path.join(
            os.path.dirname(os.path.abspath(__file__)), "leaderboard.txt"
        )
    ...
    def get_deck(self):
        ranks = [str(i) for i in range(2, 11)] + self.face_cards
    ...
    def deal_cards(self):
        ...
        self.deck.pop(0)
        self.dealer_cards.append(self.upside_down_card)
        self.show_table()
    ...
    def get_points(self, cards, points):
        ...
        for card in cards:
            if card.rank == "A":
                points += 11
                aces += 1
            elif card.rank in self.face_cards:
                points += 10
            else:
    ...
    def player_turn(self, first_round=True):
        while self.player_points < 21:
            ...
            if (
                ...
                or (
                    self.player_cards[0] in self.face_cards + [10]
                    and self.player_cards[1] in self.face_cards + [10]
                )
            ):
                options += "t - Split\n"
            ...
            while choice not in options_list:
                ...
                choice = input(options + ":").lower()
            if choice == "h":
                self.player_points = self.draw_card(
                    self.player_cards, self.player_points
                )
                if self.player_points < 21:
                    self.show_table()
            elif choice == "s":
                ...
                self.player_points = self.get_points(
                    self.player_cards, self.player_points
                )
                ...
                if self.dealer_card_2.rank in self.face_cards[:4] + [10]:
                    self.money += self.bet
    ...
    def win_conditions(self, split=False):
        ...
        while True:
            ...
            if choice == "d":
                ...
                self.score = str(max(self.money - self.initial_bank, 0))
                print(
...
The side with the highest hand value that doesn't exceed 21 wins."""
    ...
    def read_leaderboard(self):
        content_dict = {}
        try:
            with open(self.file_path, "r") as leaderboard:
    ...
    def write_leaderboard(self):
        ...
        with open(self.file_path, "w+") as leaderboard:
